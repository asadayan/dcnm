...
def get_switch_role(serialNo):
    ...
    posturl = url + f'/rest/control/switches/roles?serialNumber={serialNo}'
...
def vrf(fabricName,vrfName):
    ...
    dcnm_token = dcnm_auth.auth(url, username, password)
    headers = {'Dcnm-Token': dcnm_token, 'Content-Type': 'application/json'}
    posturl = url + f'/rest/top-down/fabrics/{fabricName}/vrfs/{vrfName}'
    response = requests.get(posturl, verify=False, headers=headers)
    if response.status_code == 200:
        output = json.loads(response.text)
...
def get_leaf_nodes(fabricName,raw=0):
    ...
    posturl = url + f'/rest/control/fabrics/{fabric_id}/inventory'
    try:
        response = requests.get(posturl, verify=False, headers=headers)
        if response.status_code == 200:
            output = json.loads(response.text)
...
def get_spine_serial_num(spine_nodes,spine):
    for item in spine_nodes:
        if item['name'] == spine:
            return item['serial_no']
...
def get_interface(fabricName):
    ...
    password = dcnm_credentials.password
    url = 'https://' + dcnm_credentials.node_ip
    posturl = url + f'/rest/globalInterface/'
    dcnm_token = dcnm_auth.auth(url, username, password)
    headers = {'Dcnm-Token': dcnm_token, 'Content-Type': 'application/json'}
    response = requests.get(posturl, verify=False, headers=headers)
    if response.status_code == 200:
...
def save_config(fabricName):
    ...
    dcnm_token = dcnm_auth.auth(url, username, password)
    headers = {'Dcnm-Token': dcnm_token, 'Content-Type': 'application/json'}
    save_url = url + f'/rest/control/fabrics/{fabricName}/config-save'
...
def deploy_config(fabricName):
    ...
    dcnm_token = dcnm_auth.auth(url, username, password)
    headers = {'Dcnm-Token': dcnm_token, 'Content-Type': 'application/json'}
    deploy_cfg = url + f'/rest/control/fabrics/{fabricName}/config-deploy'
